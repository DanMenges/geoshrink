(function () {
  ...
  function pickCapitalTarget(ctx) {
  ...
  function nextRound(ctx) {
    ...
    ctx.state.targetIdx = pickCapitalTarget(ctx);
  ...
  function onMapClick(ctx, idx) {
    ...
    ctx.scheduleTimeout(() => {
      ctx.hud.showRoundResult({
        ...
        sub: (meta.capital || 'That capital') + ' is the capital of ' + ctx.data.names[ctx.state.targetIdx] + '.',
  ...
  const mode = {
    ...
    setup(ctx) {
      ...
      ctx.hud.setPanel('');
      ctx.hud.setHint('Click the country whose capital is shown above.');
      nextRound(ctx);
    },
